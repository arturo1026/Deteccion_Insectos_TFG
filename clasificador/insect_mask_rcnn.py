...
class InsectsDataset(Dataset):
    ...
    def load_mask(self, image_id):
        ...
        for i in range(len(boxes)):
            ...
            eli f(box[4] == 'NC'): # nesidicoris
                masks[row_s:row_e, col_s:col_e, i] = 2
                class_ids.append(self.class_names.index('NC'))
            eli f(box[4] == 'MR'): # macrolophus
...
# train conjunto
dataset_di r ='datasets/renamed_to_numbers'
...
import random
nu m =random.randint(0, len(train_set.image_ids))
# define image id
image_id = num
# load the image
...
model_path = './logs/mask_rcnn_insects_cfg_0100.h5'
...
# definimos el modelo
model = MaskRCNN(mode='inference', model_dir='logs', config=cfg) # guardaremos el modelo en logs y le asignamos la config
# cargamos los pesos
model.load_weights('logs/mask_rcnn_insects_cfg_0100.h5', by_name=True)


import skimage

# cojemos la imagen de entrada en la que se intetara predecir que insectos hay
insect_img = skimage.io.imread("datasets/renamed_to_numbers/images/040.jpg")
...
ax = pyplot.gca()
class_names = ['WF', 'NC', 'MR']
class_id_counte r =1
for box in detected['rois']:
    detected_class_id = detected['class_ids'][class_id_counte r -1]
    y1, x1, y2, x2 = box
    # calculate width and height of the box
    width, height = x2 - x1, y2 - y1
    # create the shape
    ax.annotate(class_names[detected_class_i d -1], (x1, y1), color='black', weight='bold', fontsize=10, ha='center', va='center')
