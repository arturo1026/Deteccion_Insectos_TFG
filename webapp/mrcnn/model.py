...
def scheduler(epoch, lr):
    if epoch > 0 and epoch % 4 == 0:  # Reduce cada 4 épocas
        return lr * 0.90
    return lr
...
def identity_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = KL.Dropout(0.5)(x)  # Añadir Dropout
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = KL.Conv2D(nb_filter1, (1, 1), strides=strides,
                  name=conv_name_base + '2a', use_bias=use_bias, kernel_regularizer=l2(0.001))(input_tensor)
    x = BatchNorm(name=bn_name_base + '2a')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.Conv2D(nb_filter2, (kernel_size, kernel_size), padding='same',
                  name=conv_name_base + '2b', use_bias=use_bias, kernel_regularizer=l2(0.001))(x)
    x = BatchNorm(name=bn_name_base + '2b')(x, training=train_bn)
    x = KL.Activation('relu')(x)

    x = KL.Conv2D(nb_filter3, (1, 1), name=conv_name_base +
                  '2c', use_bias=use_bias, kernel_regularizer=l2(0.001))(x)
    x = BatchNorm(name=bn_name_base + '2c')(x, training=train_bn)

    shortcut = KL.Conv2D(nb_filter3, (1, 1), strides=strides,
                         name=conv_name_base + '1', use_bias=use_bias, kernel_regularizer=l2(0.001))(input_tensor)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Dropout(0.5))(x)  # Añadir Dropout
...
class MaskRCNN():
    ...
    def compile(self, learning_rate, momentum=None):
        """Gets the model ready for training. Adds losses, regularization,
        and metrics. Then calls the Keras compile() function."""
        self.keras_model.metrics_tensors = []
        
        # Determine the optimizer dynamically
        if self.optimizer == 'SGD':
            ...
            optimizer = keras.optimizers.Adam(
                lr=learning_rate, beta_1=self.config.BETA_1, beta_2=self.config.BETA_2,
                epsilon=self.config.EPSILON, clipnorm=self.config.GRADIENT_CLIP_NORM)
        elif self.optimizer == 'RMSprop':
            optimizer = keras.optimizers.RMSprop(
                lr=learning_rate, decay=self.config.DECAY, clipnorm=self.config.GRADIENT_CLIP_NORM)
        else:
        ...
        self.keras_model.compile(
            optimizer=optimizer,
            loss=[None] * len(self.keras_model.outputs)  # Assuming no external loss is needed
        )
...
#            self.config.NAME.lower(), now))
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            lr_callback  # Añadir el callback de la tasa de aprendizaje aquí
        ]
